...
(function (angular, _, moment) {
    ...
    $interpolate)
  {
    ...
    $scope.filter = '';
    $scope.currentPage = 1;
    $scope.pageSize = scDatatableConfig.defaultPageSize;
    $scope.pageCount = 1;
    $scope.pageItems = [];
    ...
    $scope.dataTableTexts = {
      info: null,
      filtered: null,
      previousPage: l10n.trans('datatable_previousPage'),
      nextPage: l10n.trans('datatable_nextPage'),
      search: l10n.trans('datatable_search'),
      displayRecords: l10n.trans('datatable_displayRecords'),
      noDataAvailable: l10n.trans('datatable_noDataAvailable')
    };
    var columnIndex = 0,
        isPageUpdated = false;
      ...
      if (column.data) {
        ...
        dataFunction = function (item) {
          if (column.type === 'date') {
            ...
            return value ? l10n.trans('datatable_yes') : l10n.trans('datatable_no');
          }
          else {
    ...
    var updateDataTableTexts = function () {
      $scope.dataTableTexts.filtered = $interpolate(l10n.trans('datatable_filtered'))({
        max: numberWithCommas($scope.items.length)
      });

      if ($scope.pageItems.length === 0) {
        $scope.dataTableTexts.info = l10n.trans('datatable_noDataAvailable');
        return;
      }
      $scope.dataTableTexts.info = $interpolate(l10n.trans('datatable_info'))({
        total: numberWithCommas($scope.itemsCount || $scope.sortedItems.length),
    ...
    var updatePageCount = function () {
      $scope.pageCount = Math.ceil(($scope.itemsCount || $scope.filteredItems.length) / $scope.pageSize);
      $scope.setPaging();
    };
    ...
    $scope.setCurrentPage = function (page) {
      ...
      if ($scope.query !== undefined) {
        var limit = parseInt($scope.pageSize, 10),
            offset = (page - 1) * limit;

        $scope.query({ offset: offset, limit: limit}).then(function () {
          isPageUpdated = true;
    ...
    $scope.setPageSize = function (pageSize) {
      if ($scope.query !== undefined) {
        $scope.query({ offset: 0, limit: pageSize }).then(function () {
          isPageUpdated = true;
          $scope.currentPage = 1;
